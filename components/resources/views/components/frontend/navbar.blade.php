<!-- Navbar Light -->
<nav
    class="navbar navbar-expand-lg {{ ( Cookie::get('theme_mode') == 'theme-light') ? 'navbar-light bg-white' : '' }} z-index-3 @if ($header->sticky_header) position-sticky z-index-sticky top-0 bg-default @endif">
    <div class="container">
                ...
                @foreach($menus as $key => $value)
                    ...
                    @if ( $value['type'] == 'link' )
                        ...
                        @if( count($value['children']) )
                            <li class="nav-item dropdown mx-2">
                                <a class="nav-link ps-2 d-flex justify-content-between cursor-pointer align-items-center"
                                   href="#navbarDropdownMenuChild" data-bs-toggle="dropdown"
                        ...
                        @else
                            ...
                            <li class="nav-item dropdown mx-2">
                                <a class="nav-link ps-2 d-flex justify-content-between cursor-pointer align-items-center"
                                   href="{{ ( $value['menu_items']  == 'custom' ) ? $value['url'] : route('home') . '/' . $value['url'] }}">
                                    @if ( !empty($value['icon']) )
                                        <i class="{{ $value['icon'] }} me-2"></i>
                                    @endif
                                    {{ __($value['text']) }}
                    ...
                    <li class="nav-item dropdown mx-2" wire:ignore>
                        <a class="nav-link ps-2 d-flex justify-content-between cursor-pointer align-items-center"
                           data-bs-toggle="dropdown">
                            <img src="{{ asset('assets/img/flags/' . localization()->getCurrentLocale() . '.svg') }}"
                                 alt="{{ localization()->getCurrentLocaleNative() }}" class="lang-menu me-2 my-auto">
                        ...
                        <div class="dropdown-menu dropdown-menu-animation px-2 mt-md-0 mt-lg-4">
                            @foreach(localization()->getSupportedLocales() as $localeCode => $properties)
                                <a class="dropdown-item border-radius-md mb-1" rel="alternate"
                                   hreflang="{{ $properties->key() }}"
                                   href="{{ localization()->getLocalizedURL($properties->key(), null, [], false) }}">
                                    <img src="{{ asset('assets/img/flags/' . $properties->key() . '.svg') }}"
                                         alt="{{ $properties->native() }}"
                                         class="lang-menu me-2 my-auto"> {{ $properties->native() }}
                                </a>
                            @endforeach
                        </div>
                    </li>
            ...
            <ul class="navbar-nav navbar-nav-hover">
                ...
                @if ( $general->dir_mode == true )
                    <li class="nav-item mx-2 mx-lg-auto my-2 my-lg-auto">
                        <a class="btn btn-icon btn-icon-only mb-0 me-2 bg-white btn-toggle-dir" type="button"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
                           title="{{ __('Direction Mode (LTR / RTL)') }}">
                ...
                @if ( $general->theme_mode == true )
                    <li class="nav-item mx-2 mx-lg-auto my-2 my-lg-auto">
                        <a class="btn btn-icon btn-icon-only mb-0 me-2 bg-white btn-toggle-mode" type="button"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
                           title="{{ __('Theme Mode (Light / Dark)') }}">
                            @if ( Cookie::get('theme_mode') == 'theme-light' )
                    ...
                    @if ( $value['type'] == 'button' )
                        ...
                        @if( count($value['children']) )
                            <li class="nav-item dropdown">
                                <a class="btn btn-icon dropdown-toggle me-2 {{ $value['class'] }}"
                                   href="#navbarDropdownMenuButton{{ $key }}" data-bs-toggle="dropdown"
                                   data-bs-auto-close="outside" role="button" aria-expanded="false">
                                    @if ( !empty($value['icon']) )
                        ...
                        @else
                            ...
                            <li class="nav-item mx-2 mx-lg-auto my-2 my-lg-auto">
                                <a class="btn btn-icon mb-0 me-2 {{ $value['class'] }}"
                                   href="{{ ( $value['menu_items']  == 'custom' ) ? $value['url'] : route('home') . '/' . $value['url'] }}">
                                    @if ( !empty($value['icon']) )
                                        <i class="{{ $value['icon'] }} icon"></i>
                                    @endif
                ...
                @if ( \App\Models\Admin\AuthPages::where('name', 'Login')->first()->status == true)
                    ...
                    @if ( Auth::user() )
                        ...
                        <li class="dropdown mx-2 mx-lg-auto my-2 my-lg-auto">
                            <div class="author align-items-center cursor-pointer" id="dropdownMenuUser"
                                 data-bs-toggle="dropdown" aria-expanded="false">
                                <img
                                    {{ ($general->lazy_loading == true) ? 'data-' : '' }}src="https://www.gravatar.com/avatar/{{ md5(strtolower(trim(Auth::user()->email))) }}?s=150&d=mm&r=g"
                                    class="avatar shadow {{ ($general->lazy_loading == true) ? 'lazyload' : '' }}">
                                <div class="name ps-2">
                            ...
                            <ul class="dropdown-menu dropdown-menu-animation px-2 mt-md-0 mt-lg-4"
                ...
                @elseif( Auth::user() && Auth::user()->is_admin == 1 )
                    <li class="dropdown mx-2 mx-lg-auto my-2 my-lg-auto">
                        <div class="author align-items-center cursor-pointer" id="dropdownMenuUser"
                             data-bs-toggle="dropdown" aria-expanded="false">
                            <img
                                src="https://www.gravatar.com/avatar/{{ md5(strtolower(trim(Auth::user()->email))) }}?s=150&d=mm&r=g"
                                class="avatar shadow">
                            <div class="name ps-2">
                                <span>{{ Auth::user()->fullname }}</span>
                                <div class="stats">
