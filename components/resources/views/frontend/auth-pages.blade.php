...
<head>
    ...
    <title>{{ __( ucwords( str_replace( array('-', '.'), ' ', Route::currentRouteName() ) ) ) }}
        - {{ $siteTitle }}</title>
...
{{--        <meta property="og:image:height" content="{{ Image::make($page->featured_image)->height() }}">--}}
        <meta property="og:image:alt"
              content="{{ __( ucwords( str_replace( '-', ' ', Route::currentRouteName() ) ) ) }}">
    ...
    @foreach(localization()->getSupportedLocales() as $localeCode => $properties)
        <link rel="alternate" hreflang="{{ $properties->key() }}"
